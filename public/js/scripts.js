$(document).ready(function() {
    $('.position-num').on('input', function() {
        ...
        $.post("/ajax/set_cart_num", {cartId: cartId, value: value}, function (data) {
            $('.table-row-' + cartId + ' .position-num').val($.trim(data));
            $('.table-row-' + cartId + ' .position-sum').html($.trim(data) * $('.table-row-' + cartId + ' .position-price').val());
    ...
    $('.remove-all-positions').click(function() {
        $.post("/ajax/remove_all_cart", {}, function () {
...
function rewriteAllPrice(){
    $.ajax({type: 'POST', url: '/ajax/get_cart_all_price', async: true, data:{}})
        .done(function(allPrice){
            $('.all-price').html($.trim(allPrice));
        });
}
function addToCart(itemId, quantity) {
    $.ajax({url:"/ajax/add_to_cart", data: {itemId: itemId, quantity: quantity}, type: 'POST', async: true}).done(function () {getCartNum();if ($('#order-form').length) {location.reload();}});
}
function getCartNum(){
    $.ajax({type: 'POST', url: '/ajax/get_cart_num', async: true, data:{}})
        .done(function(data){
            $('#cart-num').html($.trim(data));
        });
}
